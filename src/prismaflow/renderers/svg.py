...
class SVGRenderer:
    ...
    def render(self, layout: DiagramLayout) -> str:
        ...
        if layout.title:
            parts.append(self._render_diagram_title(layout.title, layout.width))
    ...
    def _render_diagram_title(self, title: str, width: float) -> str:
        """
        title: Render the visible diagram title.
        parameters:
          title:
            type: str
        ...
        returns:
          ...
          description: Return value.
        """
        max_chars = max(42, int((width - 80) / 7.2))
        lines = wrap_text(title, max_chars=max_chars)
        line_height = 16
        start_y = 42 - ((len(lines) - 1) * line_height / 2)
        tspans = []
        x = width / 2
        ...
        return (
            f'<text class="diagram-title" x="{x:g}" y="{start_y:g}">\n'
            + "\n".join(tspans)
            + "\n</text>"
        )
